...
function getBrands(){
	...
	if(isset($_POST['insert_post'])){
		...
		move_uploaded_file($product_image_tmp,"complain_images/$product_image");
	
		
		//writting the query for inserting this data to product table taht inside the ecomerce  database
		 $insert_product = "insert into complain (user_name,user_email,product_title,product_cat,product_brand, product_image,author_address,author_phone_number,product_desc) 

		 values ('$user_name','$user_email','$product_title','$product_cat','$product_brand',
		 '$product_image','$author_address','$author_phone_number','$product_desc')";
